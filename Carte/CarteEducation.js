...
d3.json("data/geojsonLayer.json",function(dataJSON){
	d3.csv("data/Zscores.csv", function(dataCSV) {
		...
		var maxRevenu = dataCSV[0].Zscore_educ;
		var minRevenu = dataCSV[0].Zscore_educ;
		for(var i=1; i<dataCSV.length; i++) {
			dataCSV[i].Zscore_educ = +dataCSV[i].Zscore_educ;

			if(maxRevenu<dataCSV[i].Zscore_educ) {
				maxRevenu = dataCSV[i].Zscore_educ
			}

			if(minRevenu>dataCSV[i].Zscore_educ) {
				minRevenu = dataCSV[i].Zscore_educ
			}
		}
	  	...
	  	style: function(feature) {
	  		for(i=0; i<dataCSV.length; i++) {
					if(dataCSV[i].CTNAME==feature.properties.CTNAME) {
						return {
							fillColor: colorScale(dataCSV[i].Zscore_educ),
							weight: 1,
			        opacity: 1,
			        color: 'white',
			        dashArray: '3',
